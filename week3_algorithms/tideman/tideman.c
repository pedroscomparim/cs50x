...
bool path_exists(int current, int target);
...
// Check whether there is a path from current to target
bool path_exists(int current, int target)
{
    if (current == target)
    {
    ...
    {
        if (locked[current][next])
        {
            if (path_exists(next, target))
            {
                return true;
            }
        }
